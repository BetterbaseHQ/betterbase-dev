...
Wave 0 (AUD-060, AUD-001, AUD-002, AUD-015 storage portion) completed 2026-09-20: [wave-0 evidence](evidence/2026-09-20-remediation-wave0.md). Wave 1 (AUD-003, AUD-005, AUD-006; AUD-015 route-level closure) completed 2026-09-20: [wave-1 evidence](evidence/2026-09-20-remediation-wave1.md). Wave 2a (AUD-026, AUD-028, AUD-029, AUD-030, AUD-032, AUD-034) completed 2026-09-20: [wave-2a evidence](evidence/2026-09-20-remediation-wave2a.md). Wave 2b (AUD-024, AUD-031, implementing D-005) completed 2026-09-20: [wave-2b evidence](evidence/2026-09-20-remediation-wave2b.md). Independent code review ran per wave; its should-fix findings were fixed in-wave (or recorded as explicit decisions).

## D-005: Revocation guarantees full forward secrecy at rotation

2026-09-20 — User decision (on agent recommendation) resolving the Stage-1 open guarantee "revocation timing, historical-data access, and treatment of previously downloaded keys/data" for AUD-024:

- **Guarantee**: after a member is removed and the completing rotation finishes, the removed member cannot decrypt any content created after removal — rotation keys are fresh random secrets, never derived from prior epoch keys. Already-downloaded plaintext stays readable to the removed member (accepted; inherent to E2EE).
- **Liveness design**: the rotating admin wraps the fresh key for every remaining member (admins included) and stores the wrapped copies server-side (same trust model as invitations — wrapped to each recipient's public key, server never sees plaintext). Distribution is pull-based (members fetch on next sync). Ordering: distribute wrapped copies **before** rewriting any DEKs, so a crashed originator never leaves ciphertext under a lost key; an orphaned rotation is cleared via the existing `epoch.complete` gate and restarted by any admin; after distribution, any admin can complete the rewrap using their own fetched copy.
- **Server enforcement**: revocation detaches the removed member's subscriptions for that space and preserves the connection for their other spaces (re-subscribe attempts fail authorization) — refined from "close connections" during implementation because closing the shared connection would sever the member's unrelated spaces; the guarantee (no future ciphertext delivery to the revoked socket) is unchanged. WS connections also close at JWT expiry (closes AUD-031's bounded overhang).
- Compatible evolution: additive RPCs/tables only; derived-key history (epochs ≤ e) continues to work during transition; the frozen v1 surface (routes, frame types, close codes 4000–4007) is untouched.
